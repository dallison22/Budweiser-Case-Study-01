...
From above summary and plot, I observed that the distribution of ABV is almost normally distributed with mean = 0.056 and median of 0.05968. The values of ABV range from 0.0010 to 0.12800. There are many outliers with lowest and highest values of ABV.
...
ggplot(data, aes(x = ABV, y = IBU)) +
  ...
  labs(title = "Relation b/w Beer's Bitterness and Alcoholic Content")
```
```{r}
cor(data$ABV, data$IBU)
```

From above plot, I observed that the relation between bitterness of beer and it's alcoholic content is positive, as the value of bitterness of bear increase with increase of it's alcoholic content. Since the points are spread out and not clustered around the regression line so the relation between these two variables is moderate as evident by the correlation value of 0.514. There are some outliers in the data too.
...
#### Conclusion
To conclude that, US has largest collection of breweries vary by states. Some states have multiple kind of breweries and some have only one kind of brewery. All the analysis and visualizations that are performed in this case study can be used to increase the sale of beer products, and can also be used to find out which state has highest value of IBU and ABV and which states used largest size of bottles as compare to other states. I also find out that most of the bears have ABV value of 0.04 to 0.08. And also that there is a positive relation between IBU and ABV. All this information can be used to make useful decisions in future. For example states that already have high number of breweries may not be primary candidates for shipments of beer from other States. 
